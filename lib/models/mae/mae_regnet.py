...
class XBlockMAE(nn.Module):
    ...
    #     reduce = out_channels > in_channels
    #     if reduce:
    #         stride = 2
...
class XDecoderBlockMAE(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, seq_len, out_seq, b, g, p_dropout=None, relu=True):
        ...
        # )
        self.c = nn.Sequential(
            nn.ConvTranspose1d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size//2, groups=g, stride=stride, output_padding=p),
            nn.LayerNorm((self.seq_len)),
            nn.ReLU(),
        )
        if self.use_relu:
